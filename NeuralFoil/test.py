...
airfoils_dir = r"C:\Users\camer\OneDrive\Documents\Cal Poly\Research\SeniorProject\TestingNACA\airfoil_database"
airfoils_dir_files = os.listdir(airfoils_dir)
random.shuffle(airfoils_dir_files)
for filename in airfoils_dir_files:
    af = asb.Airfoil(name=filename.split('.')[0], coordinates=os.path.join(airfoils_dir, filename)).to_kulfan_airfoil()
    ...
    plt.suptitle("Comparison of $C_L$-$C_D$ Polar for NeuralFoil vs. XFoil", fontsize=16, y=1)
    ...
    p.show_plot(
        None,
        legend=False,
        savefig=f"figs/figs_small_large/{af.name}_comparison.png",
